...
class ContainerConverter:
    def __init__(self):
        ...
        # port_codes 딕셔너리 추가
        self.port_codes = {'AEAJM': 'AJMAN', 'AEAUH': 'ABU DHABI', 'AEDXB': 'DUBAI', 'AEFJR': 'AL - FUJAYRAH', 'AEJEA': 'JEBEL ALI'}
        ...
        try:
            ...
            for line in file_lines:
                ...
                # DIM (치수 정보) 처리
                    ...
                    if edi_lines[1] == "9":
                        OH = str(int(edi_lines[2].split(":::")[1].rstrip("'")))  # Convert to int to remove leading zeros, then back to string
                    elif edi_lines[1] == "8":
                   
                        Oleft = str(int(edi_lines[2].split("::")[1].rstrip("'")))  # Convert to int to remove leading zeros, then back to string
                    elif edi_lines[1] == "7":
              
                        Oright = str(int(edi_lines[2].split("::")[1].rstrip("'")))  # Convert to int to remove leading zeros, then back to string
                    
                        print(f"OH: {OH}, Oleft: {Oleft}, Oright: {Oright}")
       
                        ws.cell(row=cntr_count, column=21, value=f"//{OH}/{Oleft}/{Oright}")
