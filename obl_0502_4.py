...
# pyinstaller -w -F --add-binary="C:/Users/kod03/AppData/Local/Programs/Python/Python311/tcl/tkdnd2.8;tkdnd2.8" obl0502_3.py
# pyinstaller -w -F --add-binary="C:/Users/kod03/AppData/Local/Programs/Python/Python311/tcl/tkdnd2.8;tkdnd2.8" --icon="ICON_3.ico" obl_0502_4.py
...
class ContainerConverter:
    ...
    def setup_ui(self):
        ...
        self.obl_editor_tab = ttk.Frame(self.tab_control)  # OBL EDITOR 탭 추가
        ...
        self.tab_control.add(self.obl_editor_tab, text='OBL EDITOR')  # OBL EDITOR 탭 추가
        ...
        self.setup_obl_editor_tab()  # OBL EDITOR 탭 설정
    ...
    def setup_stowage_tab(self):
        ...
        if self.stow_mapping:
            # [0_Dummy] Service를 항상 첫번째로 표시
            if '[0_Dummy]' in self.stow_mapping:
                preview_text += f"\nService Name: [0_Dummy]\n"
                self.preview_text.tag_configure("green_bg", background="#90EE90")
                self.preview_text.tag_add("green_bg", "end-2c linestart", "end-1c lineend")
                preview_text += "------------------------\n"
                dummy_mappings = self.stow_mapping['[0_Dummy]']
                if isinstance(dummy_mappings, list):
                    for mapping in dummy_mappings:
                        if isinstance(mapping, dict):
                            preview_text += f"Port: {mapping.get('port', 'N/A')}\n"
                            preview_text += f"Stow Code: {mapping.get('stow_code', 'N/A')}\n"
                            preview_text += "------------------------\n"

            # 나머지 서비스 표시
            for service_name, mappings in self.stow_mapping.items():
                if service_name != '[0_Dummy]':
                    preview_text += f"\nService Name: {service_name}\n"
                    self.preview_text.tag_configure("green_bg", background="#90EE90")
                    self.preview_text.tag_add("green_bg", "end-2c linestart", "end-1c lineend")
                    preview_text += "------------------------\n"
                    if isinstance(mappings, dict):
                        for port, stow_code in mappings.items():
                            preview_text += f"Port: {port}\n"
                            preview_text += f"Stow Code: {stow_code}\n"
                            preview_text += "------------------------\n"
    ...
    def drop_obl_file(self, event):
        ...
        # empty_entries가 없으면 초기화
        if not hasattr(self, 'empty_entries'):
            self.empty_entries = []

        # selected_pol과 selected_tol이 없으면 초기화
        if not hasattr(self, 'selected_pol'):
            self.selected_pol = tk.StringVar()
        if not hasattr(self, 'selected_tol'):
            self.selected_tol = tk.StringVar()
                ...
                if pod not in ["POD", ""] and sztp not in ["SzTp", ""] and qty not in ["수량", ""]:
                    try:
                        ...
                        for i in range(qty):
                            ...
                            # stow_code 설정
                            stow_code = ''
                            if hasattr(self, 'stow_mapping') and isinstance(self.stow_mapping, dict):
                                # stow_mapping이 딕셔너리인 경우
                                stow_code = self.stow_mapping.get(pod, '')
                            elif hasattr(self, 'stow_mapping') and isinstance(self.stow_mapping, list):
                                # stow_mapping이 리스트인 경우
                                for mapping in self.stow_mapping:
                                    if isinstance(mapping, dict) and mapping.get('port') == pod:
                                        stow_code = mapping.get('stow_code', '')
                                        break
                            ...
                            empty_row.update({
                                ...
                                'Fumi': 'N',
                                'VGM': 'Y',
                                'Stow': stow_code  # 수정된 stow_code 설정
                            })
        ...
        try:
            ...
            for idx, row in itps_df.iterrows():
                try:
                    ...
                    pod = self.convert_to_port_code(row['Next POD']) if pd.notna(row['Next POD']) else ''
    ...
    def update_stowage_preview(self):
        ...
        try:
            if hasattr(self, 'preview_text'):
                ...
                # 태그 설정
                self.preview_text.tag_configure("service_name", background="#90EE90")
                
                preview_text = "=== 현재 매핑 ===\n"
                self.preview_text.insert(tk.END, preview_text)
                
                if not self.stow_mapping:
                    self.preview_text.insert(tk.END, "매핑 정보가 없습니다.")
                else:
                    for service_name, mappings in self.stow_mapping.items():
                        # Service Name 라인 추가
                        service_line = f"\nService Name: {service_name}\n"
                        line_start = self.preview_text.index("end-1c")
                        self.preview_text.insert(tk.END, service_line)
                        line_end = self.preview_text.index("end-1c")
                        self.preview_text.tag_add("service_name", line_start, line_end)
                        
                        self.preview_text.insert(tk.END, "------------------------\n")
                        
                        if isinstance(mappings, dict):
                            for port, stow_code in mappings.items():
                                self.preview_text.insert(tk.END, f"Port: {port}\n")
                                self.preview_text.insert(tk.END, f"Stow Code: {stow_code}\n")
                                self.preview_text.insert(tk.END, "------------------------\n")
                        elif isinstance(mappings, list):
                            for mapping in mappings:
                                if isinstance(mapping, dict):
                                    self.preview_text.insert(tk.END, f"Port: {mapping.get('port', 'N/A')}\n")
                                    self.preview_text.insert(tk.END, f"Stow Code: {mapping.get('stow_code', 'N/A')}\n")
                                    self.preview_text.insert(tk.END, "------------------------\n")
                
        except Exception as e:
            print(f"미리보기 업데이트 중 오류 발생: {str(e)}")
            messagebox.showerror("오류", f"미리보기 업데이트 중 오류가 발생했습니다: {str(e)}")
    ...
    def find_matching_services(self, pod_list):
        ...
        for service_name, mappings in self.stow_mapping.items():
            ...
            elif not matching_services:  # matches가 없고 matching_services도 비어있으면
                matching_services['0_Dummy'] = [{'pod': pod, 'port': pod, 'stow_code': pod} for pod in pod_list]
    ...
    def process_edi_file(self, event):
        try:
            ...
            # FULL/EMPTY 구분을 위한 딕셔너리 초기화
            pod_summary_full = {}
            pod_summary_empty = {}
            
            # F/E 구분하여 집계
            for row in range(7, ws.max_row + 1):
                pod = ws.cell(row=row, column=1).value  # POD
                fe = ws.cell(row=row, column=11).value  # F/E 컬럼
                
                if pod and pod != "UNSET":
                    if fe == 'F':
                        pod_summary_full[pod] = pod_summary_full.get(pod, 0) + 1
                    elif fe == 'E': 
                        pod_summary_empty[pod] = pod_summary_empty.get(pod, 0) + 1
            
            # FULL 컨테이너 출력
            self.pod_summary_text.insert(tk.END, "=== FULL ===\n")
            total_full = 0
            for pod, count in sorted(pod_summary_full.items()):
                if pod == "KRPUS":
                    ...
                    self.pod_summary_text.insert(tk.END, f"{pod}: {count}\n")
                total_full += count
            self.pod_summary_text.insert(tk.END, f"Total FULL: {total_full}\n\n")
            
            # EMPTY 컨테이너 출력
            self.pod_summary_text.insert(tk.END, "=== EMPTY ===\n")
            total_empty = 0
            for pod, count in sorted(pod_summary_empty.items()):
                if pod == "KRPUS":
                    self.pod_summary_text.insert(tk.END, f"{pod}: {count}\n", "krpus")
                elif pod == "KRKAN":
                    self.pod_summary_text.insert(tk.END, f"{pod}: {count}\n", "krkan")
                    ...
                    self.pod_summary_text.insert(tk.END, f"{pod}: {count}\n")
                total_empty += count
            self.pod_summary_text.insert(tk.END, f"Total EMPTY: {total_empty}\n")
            self.pod_summary_text.insert(tk.END, f"Total Containers: {total_full + total_empty}\n\n")

            # POL 기준 POD Summary 출력 (F/E 구분)
            self.pod_summary_text.insert(tk.END, f"=== POD Summary (From {port}) ===\n\n")
            
            # POL 기준 F/E 구분 딕셔너리 초기화
            pol_pod_summary_full = {}
            pol_pod_summary_empty = {}
            
            # POL 기준 F/E 구분하여 집계
            for row in range(7, ws.max_row + 1):
                pod = ws.cell(row=row, column=1).value  # POD
                pol = ws.cell(row=row, column=5).value  # POL
                fe = ws.cell(row=row, column=11).value  # F/E
                
                if pod and pod != "UNSET" and pol == port:
                    if fe == 'F':
                        pol_pod_summary_full[pod] = pol_pod_summary_full.get(pod, 0) + 1
                    elif fe == 'E':
                        pol_pod_summary_empty[pod] = pol_pod_summary_empty.get(pod, 0) + 1
            
            # POL FULL 컨테이너 출력
            self.pod_summary_text.insert(tk.END, f"=== FULL (From {port}) ===\n")
            pol_total_full = 0
            for pod, count in sorted(pol_pod_summary_full.items()):
                if pod == "KRPUS":
                    self.pod_summary_text.insert(tk.END, f"{pod}: {count}\n", "krpus")
                elif pod == "KRKAN":
                    self.pod_summary_text.insert(tk.END, f"{pod}: {count}\n", "krkan")
                    ...
                    self.pod_summary_text.insert(tk.END, f"{pod}: {count}\n")
                pol_total_full += count
            self.pod_summary_text.insert(tk.END, f"Total FULL from {port}: {pol_total_full}\n\n")
            
            # POL EMPTY 컨테이너 출력
            self.pod_summary_text.insert(tk.END, f"=== EMPTY (From {port}) ===\n")
            pol_total_empty = 0
            for pod, count in sorted(pol_pod_summary_empty.items()):
                if pod == "KRPUS":
                    self.pod_summary_text.insert(tk.END, f"{pod}: {count}\n", "krpus")
                elif pod == "KRKAN":
                    self.pod_summary_text.insert(tk.END, f"{pod}: {count}\n", "krkan")
                    ...
                    self.pod_summary_text.insert(tk.END, f"{pod}: {count}\n")
                pol_total_empty += count
            self.pod_summary_text.insert(tk.END, f"Total EMPTY from {port}: {pol_total_empty}\n")
            self.pod_summary_text.insert(tk.END, f"Total from {port}: {pol_total_full + pol_total_empty}")
    ...
    def setup_asc_parser_tab(self):
        ...
        self.asc_summary_text = tk.Text(summary_frame, height=30, width=40)
        self.asc_summary_text.pack(pady=5, padx=5, fill="both", expand=True)
        self.asc_summary_text.insert(tk.END, "ASC Parser 탭에서 파일 변환 시 Summary가 표시됩니다.")
    ...
    def process_asc_file(self, file_path):
        ...
        # 헤더 정보 맵핑
        vessel = header_parts[1].strip()  # MSC JADE
        voy = header_parts[2].strip()     # FT512E
        port = header_parts[4].strip()    # POD:IST
        formatted_date_time = header_parts[5].strip()  # 20250323
        
        # 데이터 파싱
        data = []
        msc_containers = []  # OPR이 MSC인 컨테이너
        opr_summary = {}  # OPR별 통계
        msc_pod_summary = {}  # MSC 컨테이너의 POD별 통계
        msc_pol_summary = {}  # MSC 컨테이너의 POL별 통계
        msc_pol_pod_summary = {}  # MSC 컨테이너의 POL-POD별 통계


# 250504 --> text[0:6]
# MSNU2889751 --> text[7:17] # MSNU2889751
# MSCCNXMNTUATEK --> text[18:32]
# TEKROCND20DV245 = text[36:50]

        # "***Refer to the following remark." 문자열을 찾을 때까지 파싱
        for line in lines[2:]:  # 첫 번째와 두 번째 줄은 헤더이므로 건너뜁니다
            if "***Refer to the following remark." in line:
                break
                
            if line.strip():
                parts = [
                    line.strip()[0:6],    # 250504
                    line.strip()[7:17],   # MSNU2889751 
                    line.strip()[19:32],  # MSCCNXMNTUATEK
                    line.strip()[36:53]   # BKTTEKROCND20DV245
                ]
                if len(parts) >= 4:  # 최소 필요한 필드 수 확인
                    try:
                        # 세 번째 필드에서 OPR, POL, POD, FPOD 추출
                        third_field = parts[2]
                        fourth_field = parts[3]

                        opr = third_field[:3]  # MSK
                        pol = fourth_field[3:8]  # TRIST
                        pod = third_field[3:8]  # ISTPS
                        fpod = pod  # E
                        
                        # 네 번째 필드에서 POR, FPOD, TpSz, WGT, F/E 추출
                       
                        por = pol  # PSE
                        fpod2 = fourth_field[3:8]  # KWSAA
                        
                        # TpSz, WGT, F/E 추출
                     
                        tpsz = fourth_field[-9:-5]# 40HC
                        wgt = float(round(int(fourth_field[12:15])/10, 1)) # 13.0
                        fe = fourth_field[-2]  # F
                        
                        container_info = {
                            'POD': pod,                # ISTPS                            
                            'CELL': int(parts[0]),           # 620718
                            'Cntr No.': str(parts[1]),      # MSKU0163831
                            'OPR': opr,                # MSK
                            'POL': pol,                # TRIST
                            'STOW': '',                # ISTPS
                            'FPOD': fpod,              # E
                        ...
                        # OPR별 통계 업데이트
                        opr_summary[opr] = opr_summary.get(opr, 0) + 1
                        
                        # OPR에 따라 분류
                        if opr == 'MSC':
                            msc_containers.append(container_info)
                            # POD별 통계 업데이트
                            msc_pod_summary[pod] = msc_pod_summary.get(pod, 0) + 1
                            # POL별 통계 업데이트 (MSC 컨테이너만)
                            msc_pol_summary[pol] = msc_pol_summary.get(pol, 0) + 1
                            # POL-POD별 통계 업데이트
                            if pol not in msc_pol_pod_summary:
                                msc_pol_pod_summary[pol] = {}
                            msc_pol_pod_summary[pol][pod] = msc_pol_pod_summary[pol].get(pod, 0) + 1
        ...
        # Summary 정보 생성
        summary_text = "=== ASC File Summary ===\n\n"
        summary_text += f"Total Containers: {len(data)}\n\n"
        
        # OPR별 통계 추가
        summary_text += "=== Containers by OPR ===\n"
        sorted_oprs = sorted(opr_summary.items(), key=lambda x: x[1], reverse=True)
        for opr, count in sorted_oprs:
            summary_text += f"{opr}: {count}\n"
            
            # F/E 별 통계
            fe_summary = {}
            for container in data:
                if container['OPR'] == opr:
                    fe = container['F/E']
                    fe_summary[fe] = fe_summary.get(fe, 0) + 1
            
            sorted_fe = sorted(fe_summary.items())
            for fe, fe_count in sorted_fe:
                summary_text += f"  {fe}: {fe_count}\n"
                
                # TpSz 별 통계 
                tpsz_summary = {}
                for container in data:
                    if container['OPR'] == opr and container['F/E'] == fe:
                        tpsz = container['TpSz']
                        tpsz_summary[tpsz] = tpsz_summary.get(tpsz, 0) + 1
                        
                sorted_tpsz = sorted(tpsz_summary.items())
                for tpsz, tpsz_count in sorted_tpsz:
                    summary_text += f"    {tpsz}: {tpsz_count}\n"
            
            summary_text += "\n"
        
        # MSC POD별 통계 추가
        if msc_containers:
            summary_text += "\n=== MSC Containers by POD ===\n"
            
            # 특별한 POD 목록
            special_pods = ['KRPUS', 'KRKAN', 'KRINC']
            
            # POD별 통계를 정렬하여 표시
            sorted_pods = sorted(msc_pod_summary.items(), key=lambda x: x[1], reverse=True)
            for pod, count in sorted_pods:
                # 특별한 POD인 경우 색상 태그 추가
                if pod in special_pods:
                    summary_text += f"<span style='background-color:#FFD700;color:#000000;font-weight:bold'>{pod}: {count}</span>\n"
                else:
                    summary_text += f"{pod}: {count}\n"
        
        # MSC POL별 통계 추가 (POD로 세분화)
        if msc_containers:
            summary_text += "\n=== Containers by POL ===\n"
            sorted_pols = sorted(msc_pol_summary.items(), key=lambda x: x[1], reverse=True)

            special_pods = ['KRPUS', 'KRKAN', 'KRINC']

            for pol, total_count in sorted_pols:
                summary_text += f"\nPOL {pol}: {total_count}\n".replace(f"POL {pol}: {total_count}", f"**POL {pol}: {total_count}**")  # POL 앞에 빈 줄 추가
                # 해당 POL의 POD별 통계 추가
                if pol in msc_pol_pod_summary:
                    sorted_pods = sorted(msc_pol_pod_summary[pol].items(), key=lambda x: x[1], reverse=True)
                    for pod, count in sorted_pods:
                        # 특별한 POD인 경우 색상 태그 추가
                        if pod in special_pods:
                            summary_text += f"<span style='background-color:#FFD700;color:#000000;font-weight:bold'>{pod}: {count}</span>\n"
        ...
        # 색상 태그를 지원하는 텍스트 위젯으로 변경
        self.asc_summary_text.tag_configure("highlight", background="#FFD700", foreground="#000000", font=("Courier", 10, "bold"))
        
        # 태그를 적용하여 텍스트 삽입
        for line in summary_text.split('\n'):
            if line.startswith("<span style='background-color:#FFD700;color:#000000;font-weight:bold'>"):
                # 태그가 있는 라인 처리
                pod_info = line.replace("<span style='background-color:#FFD700;color:#000000;font-weight:bold'>", "").replace("</span>", "")
                self.asc_summary_text.insert(tk.END, pod_info + "\n", "highlight")
            else:
                # 일반 라인 처리
                self.asc_summary_text.insert(tk.END, line + "\n")
        ...
        # 데이터프레임을 엑셀 시트에 쓰기
        for row_idx, row in enumerate(df.values, 7):  # 7번째 행부터 데이터 시작
    ...
    def setup_obl_editor_tab(self):
        """OBL EDITOR 탭 설정"""
        # 좌우 분할
        left_frame = ttk.Frame(self.obl_editor_tab)
        left_frame.pack(side=tk.LEFT, fill="both", expand=True)
        
        right_frame = ttk.Frame(self.obl_editor_tab)
        right_frame.pack(side=tk.RIGHT, fill="both", expand=True)

        # 왼쪽: OBL 파일 드래그 앤 드롭 영역
        drop_frame = ttk.LabelFrame(left_frame, text="OBL 파일 드래그 앤 드롭")
        drop_frame.pack(fill="both", expand=True, pady=5)

        self.obl_editor_drop_label = ttk.Label(
            drop_frame,
            text="OBL 파일을 여기에 드롭하세요",
            font=('Arial', 12)
        )
        self.obl_editor_drop_label.pack(fill="both", expand=True, padx=20, pady=20)

        # 드래그 앤 드롭 바인딩
        self.obl_editor_drop_label.drop_target_register(DND_FILES)
        self.obl_editor_drop_label.dnd_bind('<<Drop>>', self.drop_obl_for_editor)
        ...
        ttk.Label(pod_frame, text="새로운 POD:").pack(side=tk.LEFT, padx=5)
        self.new_pod_entry = ttk.Entry(pod_frame)
        self.new_pod_entry.pack(side=tk.LEFT, fill="x", expand=True, padx=5)
        
        # 버튼 영역
        button_frame = ttk.Frame(edit_frame)
        button_frame.pack(fill="x", padx=5, pady=10)
        
        ttk.Button(button_frame, text="컨테이너 유지", command=self.keep_containers).pack(side=tk.LEFT, padx=5)
    ...
    def update_container_count(self, event=None):
        """컨테이너 수 업데이트"""
        if self.container_text.edit_modified():  # 텍스트가 수정되었는지 확인
            # 텍스트 내용 가져오기
            text_content = self.container_text.get(1.0, tk.END).strip()
            # 빈 줄을 제외한 라인 수 계산
            container_count = len([line for line in text_content.split('\n') if line.strip()])
            # 카운트 레이블 업데이트
            self.container_count_label.config(text=f"컨테이너 수: {container_count}")
            # Modified 플래그 리셋
            self.container_text.edit_modified(False)

    def drop_obl_for_editor(self, event):
        """OBL 파일 드롭 처리 (EDITOR)"""
        file_path = event.data.strip('{}').strip('"')
        if not os.path.exists(file_path):
            messagebox.showerror("오류", "파일이 존재하지 않습니다.")
        ...
        try:
            # 파일 경로 저장
            self.obl_editor_file_path = file_path
            
            # 상태 업데이트
            self.obl_editor_status_label.config(text=f"파일 로드됨: {os.path.basename(file_path)}")
            self.obl_editor_drop_label.config(text=f"현재 파일: {os.path.basename(file_path)}")
    ...
    def keep_containers(self):
        """선택한 컨테이너만 유지"""
        if not self.obl_editor_file_path:
            messagebox.showwarning("경고", "먼저 OBL 파일을 로드해주세요.")
            return
        
        # 텍스트 영역에서 컨테이너 번호 추출
        container_text = self.container_text.get(1.0, tk.END).strip()
        if not container_text:
            messagebox.showwarning("경고", "유지할 컨테이너 번호를 입력해주세요.")
            return
        
        # 줄바꿈으로 분리하여 컨테이너 번호 리스트 생성
        container_numbers = [line.strip() for line in container_text.split('\n') if line.strip()]
        
        if not container_numbers:
            messagebox.showwarning("경고", "유효한 컨테이너 번호가 없습니다.")
            return
        
        try:
            # 엑셀 파일 열기
            wb = openpyxl.load_workbook(self.obl_editor_file_path)
            ws = wb.active
            
            # 컨테이너 번호 컬럼은 B열 (2번째 열)
            container_col = 2
            
            # 헤더 확인
            header_value = ws.cell(row=1, column=container_col).value
            if header_value != 'CtrNbr':
                messagebox.showerror("오류", "컨테이너 번호 컬럼(CtrNbr)을 찾을 수 없습니다.")
                return
            
            # 유지할 행 인덱스 찾기
            rows_to_keep = [1]  # 헤더 행은 항상 유지
            for row in range(2, ws.max_row + 1):
                cell_value = ws.cell(row=row, column=container_col).value
                if cell_value:
                    cell_value = str(cell_value).strip()
                    for container_no in container_numbers:
                        if container_no in cell_value:
                            rows_to_keep.append(row)
                            break
            
            if len(rows_to_keep) == 1:  # 헤더만 있는 경우
                messagebox.showwarning("경고", "입력한 컨테이너 번호를 찾을 수 없습니다.")
                return
            
            # 확인 메시지
            rows_to_delete = ws.max_row - len(rows_to_keep)
            if not messagebox.askyesno("확인", f"{len(rows_to_keep)-1}개의 컨테이너를 유지하고 나머지 {rows_to_delete}개를 삭제하시겠습니까?"):
                return
            ...
            # 선택된 행만 복사
            for new_row, old_row in enumerate(rows_to_keep, 1):
                for col in range(1, ws.max_column + 1):
                    new_ws.cell(row=new_row, column=col, value=ws.cell(row=old_row, column=col).value)
            
            # 새 파일명 생성
            file_dir = os.path.dirname(self.obl_editor_file_path)
            file_name = os.path.basename(self.obl_editor_file_path)
            file_name_without_ext = os.path.splitext(file_name)[0]
            
            # 현재 시간을 파일명에 추가
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            new_file_name = f"{file_name_without_ext}_kept_{timestamp}.xlsx"
            ...
            # 상태 업데이트
            self.obl_editor_status_label.config(text=f"{len(rows_to_keep)-1}개의 컨테이너 유지됨. 파일 저장됨: {new_file_name}")
            
            # 성공 메시지 표시
            messagebox.showinfo("성공", f"{len(rows_to_keep)-1}개의 컨테이너가 유지되었습니다.\n파일이 저장되었습니다: {new_file_name}")
            
        except Exception as e:
            messagebox.showerror("오류", f"컨테이너 유지 중 오류가 발생했습니다: {str(e)}")
    ...
    def delete_containers(self):
        ...
        container_text = self.container_text.get(1.0, tk.END).strip()
        if not container_text:
            messagebox.showwarning("경고", "컨테이너 번호를 입력해주세요.")
            return
        ...
        try:
            ...
            # 삭제할 행 인덱스 찾기
            rows_to_delete = []
            for row in range(2, ws.max_row + 1):  # 헤더 행은 건너뛰기
                cell_value = ws.cell(row=row, column=container_col).value
                if cell_value:
                    cell_value = str(cell_value).strip()
                    for container_no in container_numbers:
            ...
            # 확인 메시지
            if not messagebox.askyesno("확인", f"{len(rows_to_delete)}개의 컨테이너를 삭제하시겠습니까?"):
                return
            
            # 행 삭제 (역순으로 삭제하여 인덱스 변화 방지)
            ...
            # 현재 시간을 파일명에 추가
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            new_file_name = f"{file_name_without_ext}_deleted_{timestamp}.xlsx"
            new_file_path = os.path.join(file_dir, new_file_name)
            ...
            # 상태 업데이트
            self.obl_editor_status_label.config(text=f"{len(rows_to_delete)}개의 컨테이너 삭제됨. 파일 저장됨: {new_file_name}")
            
        except Exception as e:
            messagebox.showerror("오류", f"컨테이너 삭제 중 오류가 발생했습니다: {str(e)}")

    def change_pods(self):
        """선택한 컨테이너들의 POD 변경"""
        if not self.obl_editor_file_path:
            messagebox.showwarning("경고", "먼저 OBL 파일을 로드해주세요.")
            return
        ...
        # 새 POD 값 가져오기
        new_pod = self.new_pod_entry.get().strip()
        if not new_pod:
            messagebox.showwarning("경고", "새로운 POD 값을 입력해주세요.")
            return
        ...
        try:
            ...
            for col in range(1, ws.max_column + 1):
                ...
                if cell_value:
                    if 'CtrNbr' in str(cell_value):
                        container_col = col
                    elif 'POD' in str(cell_value):
                        pod_col = col
            
            if container_col is None or pod_col is None:
                messagebox.showerror("오류", "컨테이너 번호 또는 POD 컬럼을 찾을 수 없습니다.")
                return
            
            # 변경할 행 인덱스 찾기
            rows_to_change = []
            for row in range(2, ws.max_row + 1):  # 헤더 행은 건너뛰기
                cell_value = ws.cell(row=row, column=2).value  # B열 (CntrNbr)
                if cell_value:
                    cell_value = str(cell_value).strip()
                    for container_no in container_numbers:
                        if container_no in cell_value:
            ...
            # 확인 메시지
            if not messagebox.askyesno("확인", f"{len(rows_to_change)}개의 컨테이너의 POD를 '{new_pod}'로 변경하시겠습니까?"):
                return
            
            # POD 변경 (H열)
            for row in rows_to_change:
                ws.cell(row=row, column=8, value=new_pod)  # H열은 8번째 열
            ...
            # 현재 시간을 파일명에 추가
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            new_file_name = f"{file_name_without_ext}_pod_changed_{timestamp}.xlsx"
            new_file_path = os.path.join(file_dir, new_file_name)
            ...
            # 상태 업데이트
            self.obl_editor_status_label.config(text=f"{len(rows_to_change)}개의 컨테이너의 POD가 '{new_pod}'로 변경됨. 파일 저장됨: {new_file_name}")
            
            # 성공 메시지 표시
            messagebox.showinfo("성공", f"{len(rows_to_change)}개의 컨테이너의 POD가 성공적으로 '{new_pod}'로 변경되었습니다.\n파일이 저장되었습니다: {new_file_name}")
            
        except Exception as e:
            messagebox.showerror("오류", f"POD 변경 중 오류가 발생했습니다: {str(e)}")

    def save_obl_editor_file(self):
        """OBL 파일 저장"""
        if self.obl_editor_df is None or self.obl_editor_file_path is None:
            return
        ...
        try:
            ...
            # 현재 시간을 파일명에 추가
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            new_file_name = f"{file_name_without_ext}_edited_{timestamp}.xlsx"
            new_file_path = os.path.join(file_dir, new_file_name)
            
            # 파일 저장
            self.obl_editor_df.to_excel(new_file_path, index=False)
            
            # 상태 업데이트
            self.obl_editor_status_label.config(text=f"파일 저장됨: {new_file_name}")
