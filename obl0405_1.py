            ...
            if line.strip():
                ...
                if len(parts) >= 4:  # 최소 필요한 필드 수 확인
                    try:
                        ...
                        pol = third_field[3:8]  # TRIST
                        pod = third_field[8:13]  # ISTPS
                        fpod = third_field[13:18]  # E
                        
                        # 네 번째 필드에서 POR, FPOD, TpSz, WGT, F/E 추출
                        fourth_field = parts[3]
                        por = fourth_field[:3]  # PSE
                        fpod2 = fourth_field[3:8]  # KWSAA
                        
                        # TpSz, WGT, F/E 추출
                        tpsz_wgt_fe = fourth_field[8:]  # 40HC130F
                        tpsz = tpsz_wgt_fe.split('HC')[0] + 'HC'  # 40HC
                        wgt = tpsz_wgt_fe.split('HC')[1][:-1]  # 130
                        fe = tpsz_wgt_fe[-1]  # F
                        
                        container_info = {
                            'CELL': parts[0],           # 620718
                            'Cntr No.': parts[1],      # MSKU0163831
                            'OPR': opr,                # MSK
                            'POL': pol,                # TRIST
                            'POD': pod,                # ISTPS
                            'FPOD': fpod,              # E
